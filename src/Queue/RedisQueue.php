...
class RedisQueue implements QueueInterface {
	...
	public function __construct($config) {
		...
		$this->timerKey = $config['name'] . 'TimerQueue';
	...
	public function timeradd($url, $options = []) {
		$queue = serialize([
			'url' => $url,
			'options' => $options,
		]);
		$this->getInstance()->rPush($this->timerKey, $queue);
	}
	public function timernext() {
		$queue = $this->getInstance()->lPop($this->timerKey);
